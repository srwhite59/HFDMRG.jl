...
Every route returns `(psiup, psidn, energy)`. Orbital columns are in the full
physical basis and correspond to the last completed sweep. For RHF,
`psiup == psidn`. The energy is electronic and uses the conventions in
[Numerical conventions](@ref).
